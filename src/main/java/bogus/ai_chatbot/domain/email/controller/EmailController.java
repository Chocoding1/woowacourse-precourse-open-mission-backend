...
public class EmailController {
    ...
    @Operation(summary = "이메일 인증 코드 전송", description = "이메일을 파라미터로 보내면 해당 이메일로 인증 코드 전송",
            responses = {
    ...
    @Operation(summary = "이메일 인증 코드 검증", description = "이메일과 인증 코드를 파라미터로 보내면 해당 이메일로 인증 코드 검증",
            responses = {
                    @ApiResponse(responseCode = "200", description = "이메일 인증 코드 검증 완료"),
                    @ApiResponse(responseCode = "404", description = "존재하지 않는 인증 코드"),
                    @ApiResponse(responseCode = "400", description = "일치하지 않는 인증 코드")
            })
